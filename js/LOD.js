var LOD = function () {
	...
	this.updateTree = function (tree, gl, prgObj, mvpMatrix) {
		...
		(function traverse(node) {
			...
			if ( node.getNumReadyChildren() == 4 ) {
				...
				if ( dpxl.maxCellSize > acceptableCellSizeMax && dpxl.isInsideFrustum && node.level > 0 ) {
					// lock tree
					if ( tree.numNodes < acceptableNumNodes ) {
						// unlock tree
						// lock node
		...
		function deltaPixels(node, mvpMatrix, viewportWidth, viewportHeight) {
			...
			var  szX = (node.dim.x - 1) * cszX;
			var  szY = (node.dim.y - 1) * cszY;
			...
			for (var corner=0; corner < 4; ++corner) { // find the maximum value of 4 corners
				...
				//
				var varound = [
					[ox+cszX*(corner&1?1:-1), oy                     , oz],
					[ox                     , oy+cszY*(corner&2?1:-1), oz]
				];
				// Window Coordinates
				var wo = xyz2win(ox, oy, oz, mvpMatrix, viewportWidth, viewportHeight);
				if ( wo.isInsideFrustum ) ++numInsideFrustum;
				for (var iv=0; iv < 2; ++iv) {
					...
					var vz = varound[iv][2];
					// Window Coordinates
					var wv = xyz2win(vx, vy, vz, mvpMatrix, viewportWidth, viewportHeight);
					var dwx = wv.x - wo.x;
					var dwy = wv.y - wo.y;
